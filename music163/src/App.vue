  ...
  <div id="app">
      ...
      <header>
        ...
        <div class="top">
          ...
          <svg class="topsvg left" smlns="http://www.w3.org/2000/svg" viewBox="0 0 284 50" data-reactid="6">
        ...
        <nav>
          <router-link to="/" > <span id="index" class="borderBred" v-on:click="replace('index')"> 推荐音乐</span></router-link>
          <router-link to="/hot"> <span id="hot" v-on:click="replace('hot')">热歌榜</span></router-link>
          <router-link to="/seach"> <span id="seach" v-on:click="replace('seach')">搜索</span></router-link>
        </nav>
...
<script>
  export default {
    name: 'app',
    methods:{
      replace:function (className){

        var allId = ['index','hot','seach'];
        allId.forEach(function(v){
          if(v!==className){
            var domId = document.getElementById(v);
            if(domId.classList.contains('borderBred')){
              domId.classList.remove("borderBred")
            }
          }
        })

        document.getElementById(className).classList.add("borderBred")
...
abbr,acronym { border:0; 
  ...
  .clear{display: block;clear:both}
    ...
    // @import url('./assets/sass/main.scss');//不支持外部引用，引用后无法编译
  ...
  .borderBred{color:#d43c33;border-bottom: 2px solid #d43c33;}
